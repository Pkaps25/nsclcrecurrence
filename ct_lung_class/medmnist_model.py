...
def load_mendminst_resnet50(finetune_depth: int = 0):
    model = ResNet50(3, 2)
    model = Conv3dConverter(model, i3d_repeat_axis=-3)
    weights = torch.load("/data/kaplinsp/resnet50_3D_1.pth", weights_only=True)
    model.load_state_dict(weights["net"])
    if finetune_depth == 0:
        return model
    model_blocks = [n for n, subm in model.named_children() if len(list(subm.parameters())) > 0]
    finetune_blocks = model_blocks[-finetune_depth:]
...
def load_mendminst_resnet18(finetune_depth: int = 0):
    model = ResNet18(3, 2)
    model = Conv3dConverter(model, i3d_repeat_axis=-3)
    weights = torch.load("/data/kaplinsp/resnet18_3D_1.pth", weights_only=True)
    model.load_state_dict(weights["net"])
    if finetune_depth == 0:
        return model
    model_blocks = [n for n, subm in model.named_children() if len(list(subm.parameters())) > 0]
